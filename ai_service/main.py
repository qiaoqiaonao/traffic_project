...
app = FastAPI(title="交通流量分析系统 - 推理服务", version="2.0.0")
...
# 初始化
try:
    from test_inference import RTDETRPredictor

    detector = RTDETRPredictor("ai_service/weights/rtdetr_traffic.onnx", conf_threshold=0.3)
    logger.info("RT-DETR模型加载成功")
except Exception as e:
    logger.warning(f"无法加载RT-DETR模型: {e}，使用测试模式")
...
def process_video_task(task_id: str, video_path: Path, frame_skip: int = 3,
    ...
    """处理视频并生成结果视频（OpenCV + FFmpeg 转码方案）"""
    import subprocess
    import shutil
    ...
    try:
        ...
        while True:
            ...
            if processed_count % 30 == 0:
                ...
                update_task_status(
                    ...
                    f"处理中... {frame_count}/{total_frames}帧, "
        ...
        if temp_output.exists() and temp_output.stat().st_size > 0:
            ...
            try:
                ...
                logger.info(f"使用 imageio_ffmpeg: {ffmpeg_cmd}")
            except:
                # 2. 尝试系统 PATH 中的 ffmpeg
                ffmpeg_cmd = shutil.which('ffmpeg')
                if ffmpeg_cmd:
                    logger.info(f"使用系统 ffmpeg: {ffmpeg_cmd}")

            if ffmpeg_cmd:
                try:
                    # ✅ 关键修复：FFmpeg 转码命令（浏览器兼容参数）
                    cmd = [
                        ffmpeg_cmd,
                        '-i', str(temp_output),
                        '-vcodec', 'libx264',
                        '-pix_fmt', 'yuv420p',  # ✅ 关键：浏览器只支持 YUV420P
                        '-profile:v', 'baseline',  # ✅ 关键：Baseline profile（最兼容）
                        '-level', '3.0',  # ✅ 限制复杂度
                        '-preset', 'fast',  # 编码速度
                        '-crf', '23',  # 质量（越小越好，默认23）
                        '-movflags', '+faststart',  # 支持流式播放（moov atom 前置）
                        '-an',  # 不需要音频
                        '-y',  # 覆盖输出
                        str(final_output)
                    ]

                    logger.info(f"开始 FFmpeg 转码: {' '.join(cmd)}")
                    ...
                    if result.returncode == 0 and final_output.exists() and final_output.stat().st_size > 1000:
                        logger.info(f"✅ FFmpeg 转码成功: {final_output}")
                        # 验证编码格式
                        try:
                            check_cmd = [ffmpeg_cmd, '-i', str(final_output)]
                            check_result = subprocess.run(check_cmd, capture_output=True, text=True)
                            logger.info(f"输出文件信息: {check_result.stderr[:500]}")
                        except:
                            pass
                        # 删除临时文件
                        temp_output.unlink(missing_ok=True)
                    else:
                        logger.error(f"FFmpeg 转码失败: {result.stderr}")
                        # 如果转码失败，保留临时文件作为备选
                        if not final_output.exists() or final_output.stat().st_size < 1000:
                            if final_output.exists():
                                final_output.unlink()
                            temp_output.rename(final_output)
                            logger.warning("使用原始 mp4v 编码视频（浏览器可能无法播放）")
                except Exception as e:
                    logger.error(f"FFmpeg 转码异常: {e}")
                    if temp_output.exists():
                        temp_output.rename(final_output)
                        logger.warning("使用原始 mp4v 编码视频（浏览器可能无法播放）")
            else:
                # 没有找到 ffmpeg，直接使用临时文件
                logger.warning("未找到 FFmpeg，使用原始编码（浏览器可能无法播放）")
                temp_output.rename(final_output)
        else:
        ...
        logger.info(f"✅ 视频生成完成: {final_output}, 大小: {final_output.stat().st_size} bytes")
...
async def get_result_video(task_id: str, request: Request, download: bool = False):
    """
    获取结果视频 - 支持 GET 和 HEAD 请求
    HEAD 用于浏览器获取视频元信息（文件大小等）
    """
    try:
        ...
        if request.method == "HEAD":
            ...
            headers = {
                ...
                "Accept-Ranges": "bytes",
                "Last-Modified": datetime.fromtimestamp(video_path.stat().st_mtime).strftime("%a, %d %b %Y %H:%M:%S GMT")
            }
...
@app.api_route("/api/analyze/video/{task_id}/stream", methods=["GET", "HEAD"])
async def stream_video(request: Request, task_id: str):
    """
...
@app.head("/api/analyze/video/{task_id}/stream")
async def stream_video_head(task_id: str):
    """
    HEAD 请求 - 用于获取视频元信息（文件大小等）
    """
    video_path = RESULTS_DIR / f"{task_id}_result.mp4"
...
async def analyze_single_frame(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 添加整体信息
        info_text = f"Detected: {len(detections)} objects | Time: {infer_time * 1000:.1f}ms"
...
async def cancel_task(task_id: str):
    """取消正在处理的任务"""
    if task_id in active_tasks:
        del active_tasks[task_id]  # 这会触发任务内的取消检测
...
async def health_check():
    return {
        "status": "ok",
        "model": "rtdetr_traffic" if not isinstance(detector, SimpleDetector) else "test_mode",
        "version": "2.0.0",
